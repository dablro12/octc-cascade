...
import torch
import random
class Cascade_CustomDataset(Dataset):
    def __init__(self, image_dir, mask_dir,transform=None, testing = False, seed = 627):
        ...
        self.masks = [os.path.join(mask_dir, x) for x in os.listdir(mask_dir) if x.endswith('.png')]
    def __len__(self):
...
class Test_Cascade_CustomDataset(Dataset):
...
class Test_Inpaint_CustomDataset(Dataset):
